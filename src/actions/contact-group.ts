...
export async function fetchEnrichedGroup(groupId: number): Promise<ActionResult<EnrichedGroupData>> {
  try {
    ...
    if (!session.isAdmin && !(await isGroupOwner(validGroupId, session.ownerid))) {
      console.warn("[ACCESS_DENIED] fetchEnrichedGroup", session.ownerid, validGroupId);
...
export async function createContactGroup(input: {
  ...
  try {
    ...
    console.info("[AUDIT] createContactGroup", session.ownerid, group.id);
...
export async function updateContactGroup(groupId: number, formData: FormData): Promise<ActionResult> {
  try {
    ...
    if (!session.isAdmin && !(await isGroupOwner(groupId, session.ownerid))) {
      console.warn("[ACCESS_DENIED] updateContactGroup", session.ownerid, groupId);
...
export async function deleteContactGroup(groupId: number): Promise<ActionResult> {
  try {
    ...
    if (!session.isAdmin && !(await isGroupOwner(validGroupId, session.ownerid))) {
      console.warn("[ACCESS_DENIED] deleteContactGroup", session.ownerid, validGroupId);
      return { success: false, error: "You do not have permission to delete this group" };
    }

    await deleteGroup(validGroupId);

    console.info("[AUDIT] deleteContactGroup", session.ownerid, validGroupId);
    revalidatePath("/groups");
    return { success: true };
  } catch (error) {
    const appError = transformError(error);
...
export async function addMembers(input: {
  ...
  try {
    ...
    if (!session.isAdmin && !(await isGroupOwner(validated.groupId, session.ownerid))) {
      console.warn("[ACCESS_DENIED] addMembers", session.ownerid, validated.groupId);
...
export async function removeMember(groupId: number, memberId: number): Promise<ActionResult> {
  try {
    ...
    if (!session.isAdmin && !(await isGroupOwner(validGroupId, session.ownerid))) {
      console.warn("[ACCESS_DENIED] removeMember", session.ownerid, validGroupId);
...
export async function removeMembers(groupId: number, memberIds: number[]): Promise<ActionResult<{ count: number }>> {
  try {
    ...
    if (!session.isAdmin && !(await isGroupOwner(validGroupId, session.ownerid))) {
      console.warn("[ACCESS_DENIED] removeMembers", session.ownerid, validGroupId);
      return { success: false, error: "You do not have permission to remove members from this group" };
    }
...
export async function updateNotifications(input: {
  ...
  try {
    ...
    if (!session.isAdmin && !(await isGroupOwner(validated.groupId, session.ownerid))) {
      console.warn("[ACCESS_DENIED] updateNotifications", session.ownerid, validated.groupId);
  ...
  try {
    ...
    if (!session.isAdmin) {
      ...
      if (ownerChecks.some((isOwner) => !isOwner)) {
        console.warn("[ACCESS_DENIED] sendMessage", session.ownerid, validated.groupIds);
    ...
    console.info("[AUDIT] sendMessage", session.ownerid, result.messageId, validated.groupIds);
    return { success: true, data: result };
  } catch (error) {
    const appError = transformError(error);
    return { success: false, error: appError.message };
...
export async function sendBlast(input: {
  ...
  try {
    ...
    if (!session.isAdmin) {
      console.warn("[ACCESS_DENIED] sendBlast", session.ownerid);
    ...
    console.info("[AUDIT] sendBlast", session.ownerid, result.messageId);
    return { success: true, data: result };
  } catch (error) {
    const appError = transformError(error);
    return { success: false, error: appError.message };
...
> {
  try {
    ...
    if (!session.isAdmin && !isSender && !isRecipient) {
      console.warn("[ACCESS_DENIED] fetchMessageDetail", session.ownerid, validMessageId);
...
export async function fetchRecipientPreview(groupId: number): Promise<ActionResult<RecipientCounts>> {
  try {
    ...
    if (!session.isAdmin && !(await isGroupOwner(validGroupId, session.ownerid))) {
      console.warn("[ACCESS_DENIED] fetchRecipientPreview", session.ownerid, validGroupId);
